...
2. Run the project's local validation (the command(s) declared in `AGENTS.md` — e.g. `npm test`,
...
7. If `.github/pull_request_template.md` exists, ensure every section is filled with concrete
   content and no placeholder comments remain. Otherwise write a clear, structured PR body
   (summary, changes, validation/tests run).
...
# Minimal validation gate — run the project's validation command from AGENTS.md
# (e.g. `npm test`, `cargo test`, `make check`).
...
# If a PR template exists, confirm the body fills every section (no leftover
# placeholder comments) before moving on.
