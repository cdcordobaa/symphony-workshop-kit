# Symphony Workshop Kit — Planning Persona (READ FIRST)
...
of truth is **`spec/SYMPHONY-SPEC.md`** (the canonical OpenAI Symphony service spec). Start by
reading **`spec/reading-guide.md`**. Do not confuse this target with OpenSymphony — OpenSymphony is
only the *engine* that drives the build.

## Phase 1 sequence (what you do in this kit)

1. **Read the spec** via `spec/reading-guide.md`.
2. **Run AI-DLC INCEPTION** (the workflow defined below): Requirements Analysis → Workflow Planning
   → Units Generation. Produce the working units under `aidlc-docs/inception/application-design/`.
   *This decomposition is the live workshop exercise — generate it from the spec, do not assume a
   pre-made plan.*
3. **Bridge** — run `/aidlc-to-tasks` to convert the working units into
   `docs/tasks/task-package.yaml` + task files, validated by the converter.
4. **Publish** — run `/convert-tasks-to-linear apply --project-slug <slug>` to create Linear
   milestones, issues, sub-issues, and blocker relations.
5. **Hand off to the engine** — see `engine/engine-setup.md` and `RUNBOOK.md` Phase 2.

## Available skills (slash commands)

- `/linear` — read/write Linear via repo-local GraphQL helpers (`LINEAR_API_KEY`).
- `/create-implementation-plan` — the authoritative `task-package.yaml` + task-file schema.
- `/aidlc-to-tasks` — **the bridge**: AI-DLC working units → task package (validator-gated).
- `/convert-tasks-to-linear` — validate / dry-run / publish the task package to Linear.
- `/commit` — well-formed conventional commits.
- `/debug` — trace stuck OpenSymphony runs during Phase 2.

## Boundary

- **CONSTRUCTION is NOT done in this kit.** Track per-ticket implementation status in **Linear**,
  driven by the OpenSymphony engine — not in `aidlc-docs/aidlc-state.md`.
- Keep AI-DLC documentation in `aidlc-docs/`; keep the published backlog in `docs/tasks/` + Linear.
