...
| You open a worktree to build a rebuild-run ticket and the unit is **already implemented** | `EnterWorktree` branches from `worktree.baseRef: fresh` → `origin/<default branch>` → **`main`**, which still carries the *pre-reset* M1 build (a complete `src/observability/`, `src/tracker/`, …). Branching there silently hands you the finished product and voids the exercise. Create the worktree explicitly off the rebuild branch instead: `git worktree add -b <Linear gitBranchName> .claude/worktrees/<name> origin/docs/lab-hand-driven-loop`, then `EnterWorktree` with `path:`. Found on ARK-60. |
| A blocker "looks unimplemented" in the working tree but its ticket is `Done` (or the reverse) | Stale local refs. `git fetch --all --prune` **before** judging readiness — on the ARK-60 run the local pointer showed ARK-58 unbuilt while `origin` already had PR #12 merged. Linear and git are two systems of record; check both, and trust neither alone. |
| `git merge-base --is-ancestor <feature> <base>` returns false though the PR merged | Squash merge rewrites history, so the feature tip is *never* an ancestor of the base. Compare content instead: `git diff --stat <feature> <base>` — an empty (or plan-file-only) diff means the work is in. |
| Fresh worktree: `npm test` / `tsx` fails immediately | Worktrees do not share `node_modules` with the main checkout. Run `npm install` inside the new worktree once before verifying. |
