...
### CONSTRUCTION — Phase 2: Core Conformance (PRD §5.3 deferred set, dogfood board)
> Driven by the built product itself against the Notion Dev Board (DEV-* tickets), one ticket = one PR.
- [ ] Dynamic `WORKFLOW.md` watch/reload
- [x] Exponential retry/backoff + retry queue — `src/orchestrator/retry.ts` (already on `main`)
- [ ] Continuation turns on the same thread up to `max_turns`
- [ ] Per-state concurrency caps
- [x] **Stall detection (spec §8.4 brief / §8.5 Part A) — DEV-4**: `src/agent/runner.ts` ends a run with
      no parsed agent output for `agent.stall_timeout_ms` (`turn_stalled` failure, child SIGTERM'd,
      window reset per event, `<= 0` disables); tests in `test/agent/stall.test.ts`
- [x] Startup terminal workspace cleanup — `Orchestrator.cleanupTerminalWorkspaces()` (already on `main`)
- [ ] Token/runtime accounting + rate-limit tracking
- [ ] `after_create` / `before_remove` hooks + optional workspace population

## Current Status

- **Lifecycle phase**: CONSTRUCTION — ✅ **M1: MVP Walking Skeleton COMPLETE** (all 7 units merged to `main`); now in **Phase 2: Core Conformance** (PRD §5.3 deferred set), driven by the product itself off the Notion Dev Board.
- **Phase 2 progress (2026-09-21)**: **DEV-4 stall detection implemented** on branch `dogfood/DEV-4-stall-detection` (PR opened, awaiting human review — NOT merged). `src/agent/runner.ts` now arms an inactivity window at launch and re-arms it on every parsed stream event; on expiry it SIGTERMs the child and maps the attempt to `failed` with the distinct `turn_stalled` reason, logged via the existing structured logger (`issue_id` / `issue_identifier` / `session_id`, `action=agent_stall`). `agent.stall_timeout_ms <= 0` disables detection (§5.3); `turn_timeout_ms` is untouched and remains a separate `timeout` outcome. Timers are injectable (`setTimer`/`clearTimer`, defaulting to `setTimeout`/`clearTimeout`) so `test/agent/stall.test.ts` drives them with a fake clock — no new test dependency. Verified locally: `npm ci && npm run build` clean, `npm test` = **214 tests / 213 pass / 1 skipped / 0 fail** (baseline on `main` was 208 / 207 / 1 / 0 — +6 new stall tests, none weakened or skipped). Known pre-existing defect (NOT introduced here): `npm run typecheck` reports 7 `noUncheckedIndexedAccess` errors in `test/agent/runner.test.ts` on `main` — identical before and after this change; `npm run build` (src-only, `tsconfig.build.json`) is clean.
