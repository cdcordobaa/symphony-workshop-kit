...
- **Phase-2 progress (deferred set, PRD §5.3)**: **DEV-6 — `after_create` / `before_remove` workspace hooks (spec §9.4) IMPLEMENTED** (branch `notion/DEV-6-workspace-lifecycle-hooks`, PR pending at time of writing). New `src/workspace/hooks.ts` executes a hook via `bash -lc` with `cwd` = the per-issue workspace, bounded by `hooks.timeout_ms` (default 60000, SIGTERM then SIGKILL after a 2s grace), logging action/outcome/exit code/duration and never logging hook stdout/stderr (FR21). `src/workspace/manager.ts` runs `after_create` only when `created_now === true` (never on reuse) and treats failure/timeout as FATAL to preparation — it rolls the half-built directory back and throws `WorkspaceError` `hook_failed`/`hook_timeout`, so no agent can run in a half-populated workspace; `before_remove` runs immediately before deletion and its failure/timeout is logged and ignored (removal always proceeds). `before_run`/`after_run` remain parsed-and-ignored. Safety invariants in `src/workspace/safety.ts` unchanged — hooks run INSIDE the already-confined workspace. Verified: `npm ci && npm run build` clean; `npm test` = **225 pass / 1 skipped / 0 fail** (up from 207 pass / 1 skipped; +18 tests, none weakened or skipped); `npm run smoke:workspace` PASS including the new §9.4 section (after_create ran 1x across create+reuse, hanging hook killed at 309ms with `timeout_ms=300`, failing `after_create` rolled the workspace back, `before_remove` ran before deletion).
- **Plan defect noted (DEV-6)**: the root sample `WORKFLOW.md` still ships no `hooks.after_create`, so the dogfood workspaces stay empty until an operator adds one — the hook mechanism now exists, but wiring a clone hook into the live driver config changes daemon runtime behavior and was left out of this ticket's scope deliberately.
